...
#        print(url_str)
    ...
    def multi_pubmed_hits_count(term_str, n_terms = 1):
        '''
            ...
            get_pubmed_hit_counts but is made to work
            with multi_normalized_google_distance
        '''
        term_str_encoded = urllib.parse.quote(term_str, safe='')
        res = QueryNCBIeUtils.send_query_get('esearch.fcgi',
                                             'db=pubmed&term=' + term_str_encoded)
        ...
        if res is not None:
            status_code = res.status_code
            if status_code == 200:
                res_int = [int(res_json['esearchresult']['count'])]
                if n_terms >= 2:
                    if 'errorlist' in res_json['esearchresult'].keys():
                        if 'phrasesnotfound' in res_json['esearchresult']['errorlist'].keys():
                                res_int += res_json['esearchresult']['errorlist']['phrasesnotfound']
                    else:
                        for a in range(len(res_json['esearchresult']['translationstack'])):
                            if type(res_json['esearchresult']['translationstack'][a]) == dict:
                                res_int += [int(res_json['esearchresult']['translationstack'][a]['count'])]
                            elif res_json['esearchresult']['translationstack'][a] == 'OR':
                                res_int = [res_int[0]]
                                res_int += ['null_flag']
                                return res_int
                ...
                print('HTTP response status code: ' + str(status_code) + ' for query term string {term}'.format(term=term_str))
                #print("++++++ No mesh terms exist in local database. Please update database with missing data ++++++++")
        return res_int
    ...
    def multi_normalized_google_distance(name_list, mesh_flags = None):
        ...
        if sum(mesh_flags) == len(mesh_flags):
            ...
            for a in range(len(name_list)):
                 # TODO: CACHE THIS PART AND CHECK
                # IF THE NAME OR ANYTING HAS BEEN PREVIOUS CACHED
                name = name_list[a]
                if mesh_flags[a]:
                    name += "[MeSH Terms]"
                counts += QueryNCBIeUtils.multi_pubmed_hits_count(name, n_terms = 1)
        # if you don't have mesh terms
